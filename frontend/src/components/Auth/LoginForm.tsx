...
export function LoginForm({ onSuccess, onRegisterClick }: LoginFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validate()) return;
    try {
      await login({ email, password, remember, mfaToken: requiresMFA ? mfaToken : undefined });
      onSuccess?.();
    } catch {
        ...
        <>
          <div className="relative">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-2 bg-white dark:bg-dark-900 text-neutral-400">or continue with</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button type="button" className={cn(
              ...
              'hover:bg-neutral-50 dark:hover:bg-dark-800 transition-colors'
            )}>
          ...
          <p className="text-center text-sm text-neutral-500">
            ...
            <button type="button" onClick={onRegisterClick} className="text-primary-600 dark:text-primary-400 hover:underline font-medium">
              Sign up
            </button>
          </p>
        </>
